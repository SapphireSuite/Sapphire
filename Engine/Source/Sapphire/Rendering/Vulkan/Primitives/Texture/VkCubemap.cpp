...
namespace Sa
{
	...
	{
		VkFormat format = API_GetFormat(_rawCubemap.format, _rawCubemap.channelNum);
		...
		imageBufferCreateInfos.mipMapLevels = _rawCubemap.mipLevels;
		...
		mBuffer.CopyBufferToImage(device, stagingBuffer,
			...
			_rawCubemap.channelNum,
			1u,
			6u);

		mBuffer.GenerateMipmaps(device, format, _rawCubemap.width, _rawCubemap.height, _rawCubemap.mipLevels, 6u);
		...
		const VkSamplerCreateInfo samplerCreateInfo
		{
			...
			0.0f,														// minLod.
			static_cast<float>(_rawCubemap.mipLevels),					// maxLod.
