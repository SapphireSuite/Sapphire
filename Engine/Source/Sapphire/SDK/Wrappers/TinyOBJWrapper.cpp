...
namespace Sa
{
	struct TinyOBJWrapper::Callback
	{
		std::vector<RawMesh> rawMeshes;
		uint32 meshIndex = uint32(-1);

		// Import single mesh index.
		uint32 importIndex = uint32(-1);
		...
		tinyobj::callback_t Tiny()
		{
			tinyobj::callback_t tinyCB{};

			if(!vertexLayout.get() || (vertexLayout->comps & VertexComp::Position) != VertexComp::None)
				tinyCB.vertex_cb = Callback::Position;
			
			if (!vertexLayout.get() || (vertexLayout->comps & VertexComp::Normal) != VertexComp::None)
				tinyCB.normal_cb = Callback::Normal;

			if (!vertexLayout.get() || (vertexLayout->comps & VertexComp::Texture) != VertexComp::None)
		...
		bool ImportMeshOnly() const noexcept
		{
			return importIndex != uint32(-1);
		}
		...
		static void Group(void* _userData, const char** _names, int _size)
		{
			...
			// Increment index fist (init at -1).
			++cb.meshIndex;

			// Check mesh import index only.
			if (cb.importIndex != uint32(-1) && cb.meshIndex != cb.importIndex)
				return;
	...
	bool TinyOBJWrapper::Import_Internal(Callback& _cb, const std::string& _resourcePath)
	{
		...
		bool bRes = tinyobj::LoadObjWithCallback(fstream, _cb.Tiny(), &_cb, nullptr /* &matFileReader */, &warn, &error);
		...
		// Compute Tangent.
		if (!_cb.vertexLayout.get() || (_cb.vertexLayout->comps & VertexComp::Tangent) != VertexComp::None)
		{
			for (auto it = _cb.rawMeshes.begin(); it != _cb.rawMeshes.end(); ++it)
				it->ComputeTangents();
		}
	...
	bool TinyOBJWrapper::ImportOBJ(const std::string& _resourcePath, MeshAsset& _result, const MeshImportInfos& _importInfos)
	{
		Callback cb;
		cb.importIndex = _importInfos.index;
		cb.vertexLayout = _importInfos.layout;

		if (!Import_Internal(cb, _resourcePath))
			return false;

		_result.SetRawData(Move(cb.rawMeshes[0]));
