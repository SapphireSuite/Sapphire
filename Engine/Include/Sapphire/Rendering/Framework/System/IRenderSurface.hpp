...
namespace Sa
{
	...
	public:
		...
		virtual const std::vector<IFrameBuffer*>& CreateFrameBuffers(const IRenderInstance& _instance, const IRenderPass& _renderPass,
			const RenderPassDescriptor& _renderPassDesc, uint32* _size = nullptr) = 0;
		virtual void DestroyFrameBuffers(const IRenderInstance& _instance) = 0;
	};
}
